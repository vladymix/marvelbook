...
object Constants {
    ...
    fun Context.isShowed(idCharacter:String):Boolean{
       return this.getSharedPreferences(HISTORY_SHARED,Context.MODE_PRIVATE).getBoolean("$SHOWED_KEY$idCharacter", false)
    }

    fun Context.setAsShowed(idCharacter:String){
        this.getSharedPreferences(HISTORY_SHARED,Context.MODE_PRIVATE).edit().putBoolean("$SHOWED_KEY$idCharacter", true).apply()
    }
